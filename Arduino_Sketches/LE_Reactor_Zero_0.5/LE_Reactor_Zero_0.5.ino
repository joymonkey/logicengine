...
#define STATUSLED_PIN 8
  ...
  NeoPixelBus<NeoGrbFeature, Neo800KbpsMethod> statusLED(1, STATUSLED_PIN);
  NeoPixelBus<NeoGrbFeature, Neo800KbpsMethod> frontLEDs(FrontLEDCount, FRONT_PIN);
  NeoPixelBus<NeoGrbFeature, Neo800KbpsMethod> rearLEDs(RearLEDCount, REAR_PIN);
#endif
...
void setStatusLED() {
    if (flipFlop == 0) {
#if (LEDLIB==0)
      if (adjMode == 0) statusLED[0] = 0x000002; //blue
      else if (adjMode == 1) statusLED[0] = 0x000002; //blue
      else if (adjMode == 3) statusLED[0] = 0x000200; //green
#elif (LEDLIB==1)
...
#elif (LEDLIB==2)
      if (adjMode == 0) statusLED.SetPixelColor(0, RgbColor(0, 0, 2)); //blue
      else if (adjMode == 1) statusLED.SetPixelColor(0, RgbColor(0, 0, 2)); //blue
      else if (adjMode == 3) statusLED.SetPixelColor(0, RgbColor(0, 2, 0)); //green
#endif
      //flipFlop = 1;
    }
    else {
#if (LEDLIB==0)
      if (adjMode == 0) statusLED[0] = 0x020000; //red
      else if (adjMode == 1) statusLED[0] = 0x020202; //white
      else if (adjMode == 3) statusLED[0] = 0x020200; //orangey
#elif (LEDLIB==1)
...
#elif (LEDLIB==2)
      if (adjMode == 0) statusLED.SetPixelColor(0, RgbColor(2, 0, 0)); //red
      else if (adjMode == 1) statusLED.SetPixelColor(0, RgbColor(2, 2, 2)); //white
      else if (adjMode == 3) statusLED.SetPixelColor(0, RgbColor(2, 2, 0)); //orangey
#endif
      //flipFlop = 0;
    }
}
...
void statusBlink(byte blinks, byte delayTime, byte redVal, byte greenVal, byte blueVal) {
  for ( byte x = 0; x <= blinks; x++) {
      #if (LEDLIB==0)
          statusLED[0] = CRGB(redVal, greenVal, blueVal);
          FastLED.show();
      #elif (LEDLIB==1)
          statusLED.setPixelColor(0, redVal, greenVal, blueVal);
          statusLED.show();      
      #elif (LEDLIB==2)
          statusLED.SetPixelColor(0, RgbColor(redVal, greenVal, blueVal)); 
          statusLED.Show();
      #endif
      delay(delayTime);
      #if (LEDLIB==0)
          statusLED[0] = 0x000000;
          FastLED.show();
      #elif (LEDLIB==1)
          statusLED.setPixelColor(0, 0, 0, 0); 
          statusLED.show();  
      #elif (LEDLIB==2)
          statusLED.SetPixelColor(0, RgbColor(0, 0, 0));
          statusLED.Show();
      #endif
      delay(delayTime);
  }    
...
/*void recvWithEndMarker() {
    ...
    char endMarker = '\r';
    char rc;    
    while (JEDI_SERIAL.available() > 0 && newData == false) {
        rc = JEDI_SERIAL.read();
        #if (DEBUG>0)
        ...
        if (rc != endMarker) {
            ...
            inputStr[ndx] = '\0'; // terminate the string
            ndx = 0;
            newData = true;
        }
    }
          ...
          if (inputStr[2]=='0') {
            if (deviceNum==0||deviceNum==1) {
              ...
              for ( byte LEDnum = 0; LEDnum < 40; LEDnum++) {
                #if (LEDLIB==0)                        
                      frontLEDs[LEDnum] = CRGB( 0 , 0 , 0) ; //fastLED
                #elif (LEDLIB==1)                        
                      frontLEDs.setPixelColor(LEDnum, 0 , 0 , 0);  //neoPixel
                #elif (LEDLIB==2)                        
                      frontLEDs.SetPixelColor(LEDnum, (0 , 0 , 0 )); //neoPixelBus
                #endif
              }
            }
            ...
            if (deviceNum==0||deviceNum==3) {
              ...
              for ( byte LEDnum = 0; LEDnum < RearLEDCount; LEDnum++) {
                #if (LEDLIB==0)                        
                      rearLEDs[LEDnum] = CRGB( 0 , 0 , 0) ; //fastLED
                #elif (LEDLIB==1)                        
                      rearLEDs.setPixelColor(LEDnum, 0 , 0 , 0); //neoPixel
                #elif (LEDLIB==2)                        
                      rearLEDs.SetPixelColor(LEDnum, (0 , 0 , 0 )); //neoPixelBus
                #endif
              }                
            }
          }
...
#if (LEDLIB==0)
  FastLED.addLeds<SK6812, REAR_PIN, GRB>(rearLEDs, RearLEDCount);
  FastLED.addLeds<SK6812, FRONT_PIN, GRB>(frontLEDs, FrontLEDCount);
  FastLED.addLeds<SK6812, STATUSLED_PIN, GRB>(statusLED, 1);
  statusLED[0] = 0x000200; //green
  FastLED.show();
#elif (LEDLIB==1)
  statusLED.begin();
...
#elif (LEDLIB==2)
  statusLED.Begin();
  statusLED.SetPixelColor(0, RgbColor(0, 2, 0)); //green
  statusLED.Show();
  frontLEDs.Begin();
  frontLEDs.Show();
  rearLEDs.Begin();
...
void loop() {
  ...
  if (doingSerialStuff==0) {
      ...
      #if (LEDLIB==0)
            //fastLED
            frontLEDs[LEDnum] = CRGB( allColors[0][actualColorNum(frontLEDstatus[LEDnum].colorNum)][0] , allColors[0][actualColorNum(frontLEDstatus[LEDnum].colorNum)][1] , allColors[0][actualColorNum(frontLEDstatus[LEDnum].colorNum)][2]) ;
      #elif (LEDLIB==1)
            //neoPixel
      ...
      #elif (LEDLIB==2)
            //neoPixelBus
            frontLEDs.SetPixelColor(LEDnum, RgbColor( allColors[0][actualColorNum(frontLEDstatus[LEDnum].colorNum)][0] , allColors[0][actualColorNum(frontLEDstatus[LEDnum].colorNum)][1] , allColors[0][actualColorNum(frontLEDstatus[LEDnum].colorNum)][2]    ));
      #endif
          }
        }
      }
      ...
      #if (LEDLIB==0)
            //fastLED
            rearLEDs[LEDnum] = CRGB( allColors[1][actualColorNum(rearLEDstatus[LEDnum].colorNum)][0] , allColors[1][actualColorNum(rearLEDstatus[LEDnum].colorNum)][1] , allColors[1][actualColorNum(rearLEDstatus[LEDnum].colorNum)][2]) ;
      #elif (LEDLIB==1)
            //neoPixel
      ...
      #elif (LEDLIB==2)
            //neoPixelBus
            rearLEDs.SetPixelColor(LEDnum, RgbColor( allColors[1][actualColorNum(rearLEDstatus[LEDnum].colorNum)][0] , allColors[1][actualColorNum(rearLEDstatus[LEDnum].colorNum)][1] , allColors[1][actualColorNum(rearLEDstatus[LEDnum].colorNum)][2]    ));
      #endif
          }
        }
      } 
      ...
      #if (LEDLIB==0)
        //with FastLED we have to update every LED on every loop, no seperating the front and rear...
        FastLED.show();
      #elif (LEDLIB==1)
        statusLED.show();
