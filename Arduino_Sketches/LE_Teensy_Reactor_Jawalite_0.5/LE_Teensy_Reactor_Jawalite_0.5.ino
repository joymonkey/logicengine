// Teensy Reactor 0.5
// ==================
// Initial Sketch for the Logic Engine Teensy Reactor board.
...
void loop() {
  ...
  if (adjMode>0) {
    ...
    if (adjMode==1) {
      ...
      //if brightness setting changed from this loop to last, recalculate allColors... (this is done sparingly so it doesn't delay the patterns)
      if (settings[0].frontBri!=prevBrightness) calculateAllColors(settings[0].frontPalNum,0,settings[0].frontBri);
      ...
      if (settings[0].rearBri!=prevBrightness) calculateAllColors(settings[0].rearPalNum,1,settings[0].rearBri);
